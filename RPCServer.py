...
def start_io_interrupt_monitor(new_value_callback):
    ...
    values are available to read by the way of interrupts - whereas we use
    callbacks here.
    '''

    # Thanks to stack overflow and Python 2 FAQ!
...
class IOInterruptIOC(PVGroup):
    ...
    async def keypress(self, instance, async_lib):
        ...
        print('* keypress method called by the server at startup')
        queue = async_lib.ThreadsafeQueue()

        # Start a seperate thread that monitors keyboard input, telling it to
