...
class RecordMockingIOC(PVGroup):
    ...
    B = pvproperty(value=2.0, record='ao',
                precision=3)
    # and an ai with all the bell and whistles
    ...
    async def B(self, instance, value):
        if value == 1:
            # Mocked record will pick up the alarm status simply by raising
            # an exception in the putter:
            raise ValueError('Invalid value!')

    # It's also possible to modify some of the behaviour of fields on a per-
...
if __name__ == '__main__':
    ...
    # ... but what you don't see are all of the analog input record fiels
